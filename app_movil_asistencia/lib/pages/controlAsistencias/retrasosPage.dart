...
class RetrasosPage extends StatelessWidget {
  const RetrasosPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<AsistenciasModel>>(
        future:
            Provider.of<GetControl>(context, listen: false).getRetrasos(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('No hay Atrasos existentes.'));
          } else {
